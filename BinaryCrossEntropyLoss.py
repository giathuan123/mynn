...
class BinaryCrossEntropyLoss:
    ...
    def __call__(self, data: OptTensor, label: NDArray):
        ...
        # assume data have gone through the softmax function
        assert np.allclose(data.data.sum(axis=1), 1)
        assert (np.logical_or(label == 0, label == 1)
                ).all()  # label only have 2 classes
        ...
        def backward():
            assert out_tensor
            jacobian = -np.multiply(one_hot_labels,
                                    np.reciprocal(data.data))
